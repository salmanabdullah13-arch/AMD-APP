...
(async () => {
  ...
  // Read the expected version straight from sw.js rather than hardcoding it
  // here — this assertion went stale twice (v5 vs the real v8) because every
  // CACHE_VERSION bump also had to remember to update this string.
  const expectedCacheVersion = (require('fs').readFileSync(require('path').join(__dirname, 'sw.js'), 'utf8').match(/CACHE_VERSION = "([^"]+)"/) || [])[1];
  record(`Cache is created with the versioned name (${expectedCacheVersion})`, expectedCacheVersion && cacheState.keys.includes(expectedCacheVersion) ? 'PASS' : 'FAIL', JSON.stringify(cacheState.keys));
